...
export const router = createRouter({
  ...
  routes: [
    // 根路径：默认先走学生端工作台；真正落地端会被 beforeEach 纠偏
    { path: '/', redirect: '/app/workbench', meta: { auth: true } },
    // 统一登录页（不需要 auth）
...

    {
      ...
      meta: { auth: true, roles: ['DEPT_ADMIN', 'SCHOOL_ADMIN', 'SYS_ADMIN'] },
      children: [
        ...
        { path: 'audit/tasks', component: AuditTasksView, meta: { title: '审核管理', roles: ['SCHOOL_ADMIN', 'SYS_ADMIN'] } },
        { path: 'record/detail/:id', component: AdminRecordDetailView, meta: { title: '填报详情', roles: ['SCHOOL_ADMIN', 'SYS_ADMIN'] } },
...
router.beforeEach(async (to) => {
  ...
  if (to.path === '/login') {
    ...
    if (user.isAdmin) return { path: '/admin/dashboard' }
    return { path: '/app/workbench' }
  }
