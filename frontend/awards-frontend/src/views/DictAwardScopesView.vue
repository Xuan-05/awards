...
async function load() {
  loading.value = true
  try {
    const resp = await http.get<ApiResponse<PageResult<Row>>>('/dicts/award-scopes', {
...
async function toggle(row: Row) {
  const resp = await http.post<ApiResponse<null>>(`/dicts/award-scopes/${row.id}/toggle`)
  if (resp.data.code !== 0) throw new Error(resp.data.message)
...
async function remove(row: Row) {
  ...
  const resp = await http.delete<ApiResponse<null>>(`/dicts/award-scopes/${row.id}`)
  if (resp.data.code !== 0) throw new Error(resp.data.message)
