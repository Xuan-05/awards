...
const form = reactive({ organizerName: '', sortNo: 0, remark: '' })
...
async function save() {
  if (!form.organizerName) {
    ElMessage.error('请填写主办方名称')
    return
  }
  if (editingId.value) {
    const resp = await http.put<ApiResponse<null>>(`/dicts/organizers/${editingId.value}`, form)
    if (resp.data.code !== 0) throw new Error(resp.data.message)
    ElMessage.success('已保存')
  } else {
    const resp = await http.post<ApiResponse<number>>('/dicts/organizers', form)
    if (resp.data.code !== 0) throw new Error(resp.data.message)
  ...
  <el-space direction="vertical" alignment="start" size="large" style="width: 100%">
    ...
    <el-card style="width: 100%">
      <el-table :data="rows" v-loading="loading" style="width: 100%">
        ...
        <el-table-column prop="organizerName" label="主办方名称" />
    ...
    <el-dialog v-model="dialogOpen" :title="editingId ? '编辑主办方' : '新增主办方'" width="520px">
