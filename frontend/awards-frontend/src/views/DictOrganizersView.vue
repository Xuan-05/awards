...
async function load() {
  loading.value = true
  try {
    const resp = await http.get<ApiResponse<PageResult<Row>>>('/dicts/organizers', {
...
async function save() {
  ...
  if (editingId.value) {
    const resp = await http.put<ApiResponse<null>>(`/dicts/organizers/${editingId.value}`, payload)
    if (resp.data.code !== 0) throw new Error(resp.data.message)
    ElMessage.success('已保存')
  } else {
    const resp = await http.post<ApiResponse<number>>('/dicts/organizers', payload)
    if (resp.data.code !== 0) throw new Error(resp.data.message)
...
async function toggle(row: Row) {
  const resp = await http.post<ApiResponse<null>>(`/dicts/organizers/${row.id}/toggle`)
  if (resp.data.code !== 0) throw new Error(resp.data.message)
...
async function remove(row: Row) {
  ...
  const resp = await http.delete<ApiResponse<null>>(`/dicts/organizers/${row.id}`)
  if (resp.data.code !== 0) throw new Error(resp.data.message)
...
<template>
  <div class="dict-page">
    ...
    <div class="page-header">
      <h1 class="page-title">主办方库</h1>
      <p class="page-subtitle">主办方基础数据管理</p>
    </div>
    ...
    <div class="content-area">
      <div class="table-card" v-loading="loading">
        <el-table :data="rows" style="width: 100%">
          <el-table-column prop="organizerName" label="主办方名称" min-width="220" align="left" />
    ...
    <!-- 编辑弹窗 -->
    <el-dialog v-model="dialogOpen" :title="editingId ? '编辑主办方' : '新增主办方'" width="550px" class="apple-dialog">
      <div class="form-grid">
